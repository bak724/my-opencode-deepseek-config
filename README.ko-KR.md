...
> **호환성 설명**: `~/.config/opencode`는 OpenCode의 표준 전역 구성 경로입니다. 본 저장소의 `opencode/` 하위 디렉터리에는 `agents/`, `skills/`, `AGENTS.md` 등의 파일이 있으며, 레이아웃이 OpenCode 규약을 완전히 따릅니다. 환경 변수나 심볼릭 링크로 지정하면 자동으로 인식됩니다.
...
>
> 읽기 전용 에이전트(`oracle`/`reviewer`/`explore`/`librarian`)의 진정한 읽기 전용화: `edit: deny` + bash 화이트리스트(기본 전부 deny, `git status/diff/log/show/blame/grep`, `rg` 등 읽기 전용 하위 명령만 허용; `oracle`/`reviewer`는 `/review-pr` 댓글 게시를 지원하기 위해 `gh pr view/diff`, `gh issue view`, `gh api` 추가 허용).
...
| `git-release` | 태그 릴리스: 릴리스 노트, SemVer 추론, gh release 명령 |
| `resolving-merge-conflicts` | hunk별 병합 충돌 해결: 원래 의도 추적, 새로운 동작 발명 금지, 절대 --abort 사용 안 함 |
| `handoff` | 세션을 인수인계 문서로 압축(경로 참조, 내용 복사 안 함) |
| `opencode-config` | 본 저장소 OpenCode 구성 작성 및 유지 관리(agents/skills/commands/permissions) |
| `reflect` | 지속적 개선: 마찰 발견 → 최소한의 유지 가능한 수정 제안 |
| `remove-deadcode` | 데드 코드 안전 검색 및 삭제, 삭제 전 도구체인/LSP 검증 |
| `security-review` | 병합 전 보안 감사(인젝션/XSS/SSRF/비밀 키/역직렬화/경로 순회), 보고만 하고 수정 안 함 |
| `shared-language` | 도메인 용어집 구축(CONTEXT.md), 토큰 대폭 절약 |
| `simplify` | 동작 유지 코드 단순화(oracle 분석 → 적용) |
| `spec-workflow` | 경량 스펙 주도 변경: proposal → specs → design → tasks → archive |
| `verification-planning` | 구현 전 가장 좁은 검증 경로 계획 |
...
핵심 아이디어는 [oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent)(의도 게이트, 읽기 전용 격리, 안티 패턴), [oh-my-opencode-slim](https://github.com/alvinunreal/oh-my-opencode-slim)(스케줄러 우선, 폴백 체인, 거부 계약, 프롬프트 캐시 안전, impact×confidence÷cost), [anomalyco/opencode](https://github.com/anomalyco/opencode)(구성 스키마, 스킬 체계), [cli/cli](https://github.com/cli/cli)(gh v2.97 명령 세트, rate limit, gh-aw), [OpenSpec](https://github.com/Fission-AI/OpenSpec)(델타 스펙, OPSX 액션 플로우 update/verify/네 가지 질문), [mattpocock/skills](https://github.com/mattpocock/skills)(충돌 해결 규율, 인수인계 문서), [pi](https://github.com/earendil-works/pi)(먼저 답변 후 수정, 간결한 응답, 독립 세션 수집), [deepreview](https://github.com/mechanai/deepreview)(novelty 분류 수렴, 유효 크기 라우팅, Points of Agreement)의 장점을 참고하였으며, 순수 구성으로 구현, 추가 의존성 없음.
...
- **v16-v22 (지속적 슬림화)**: 명령 29→18 (-38%), AGENTS.md 290→211 (-27%), 문장별 no-op 트리밍, 스키마 검증으로 데드 키 제거
